...
(function () {
    ...
    function Mafia(mafiachan) {
        ...
        function msg(src, mess) {
            bot.send(src, mess, mafiachan);
        }

        function msgAll(mess) {
            bot.sendAll(mess, mafiachan);
        }
        ...
        var defaultTheme = {
            ...
            roles1: [
                "bodyguard", "mafia", "inspector", "werewolf", "hooker", "villager", "truemiller",
                "villager", "mafia", "villager", "mayor"
            ],
            roles2: [
                "bodyguard", "mafia1", "mafia1", "inspector", "hooker", "villager", "mafia2",
                            ...
                            for (var tr in mafia.theme.sideTranslations) {
                                if (tr !== winSide && mafia.getPlayersForTeamS(tr) !== "") {
                                    var lp = mafia.getPlayersForTeam(tr).filter(function (x) {
                                        if (players.indexOf(x) >= 0) return false; else return true;
                                    });
                                    if (lp.length > 0) {
        ...
        this.handlers = {
            ...
            night: function () {
                ...
                for (var i in mafia.theme.nightPriority) {
                    ...
                    if ([
                        "Role", "Team"
                    ].indexOf(Action.common) != -1 && "failChance" in Action && Action.failChance > Math.random()) {
                        for (var f in names) {
                        ...
                        outer:
                            for (t in targets) {
                                for (var c in commandList) {
                                    ...
                                    if ([
                                        "kill", "protect", "inspect", "distract", "poison", "safeguard", "stalk",
                                        "convert"
                                    ].indexOf(command) == -1) {
                                        continue;
                                    }
                                    if (!mafia.isInGame(target)) {
                                        ...
                                        if ((target.guarded && command == "kill") || (target.safeguarded && [
                                            "distract", "inspect", "stalk", "poison", "convert"
                                        ].indexOf(command) !== -1)) {
        ...
        this.isMafiaAdmin = function (src) {
            return util.player.auth(src) > 0 || false;
        };
        this.isMafiaSuperAdmin = function (src) {
            return util.player.auth(src) > 1 || false;
        };
        /* END CUSTOM */
        ...
        this.commands = {
            user: {
                ...
                flashme: [
                    this.flashPlayer, "To get a alert when a new mafia game starts. Type /flashme help for more info."
                ],
        ...
        this.handleCommandOld = function (src, message, channel) {
            ...
            if (this.state == "entry") {
                ...
                if (this.isInGame(name) && this.hasCommand(name, command, "night")) {
                    ...
                    if ([
                        "Any", "Self", "OnlySelf"
                    ].indexOf(player.role.actions.night[command].target) == -1 && commandData == name) {
                ...
                if (this.isInGame(name) && this.hasCommand(name, command, "standby")) {
                    ...
                    if (target !== null) {
                        if ((commandObject.target === undefined ||
                            ["Self", "Any"].indexOf(commandObject.target) == -1) && player == target) {
                            sys.sendMessage(src, "±Hint: Nope, this wont work... You can't target yourself!", mafiachan);
                            return;
