...
YAMLDumper.prototype.dump = function (node) {
    if (!(node instanceof Object)) this.stream += ' ';
    this.dump_node(node);
}
...
YAMLDumper.prototype.dump_map = function (node) {
    ...
    for (var k in node) {
        ...
        this.dump_scalar(k);
        this.stream += ':';
        if (!(node[k] instanceof Object)) this.stream += ' ';
        if (node[k] instanceof Array) {
            this.indent_level--;
            this.dump_node(node[k]);
            this.indent_level++;
        }
        else this.dump_node(node[k]);
    }
}
...
YAMLDumper.prototype.dump_seq = function (node) {
    if (node.length == 0) return this.dump_seq_empty();
    for (var e = 0; e < node.length; e++) {
        this.print_indent_array();
        if (!(node[e] instanceof Object)) this.stream += ' ';
        if ((node[e] instanceof Object)) this.noindent = true;
...
YAMLDumper.prototype.dump_scalar = function (node) {
    if ((node == null) || (typeof(node) == 'undefined')) return this.dump_scalar_null();
    if (typeof(node) == 'boolean') return this.dump_scalar_plain(node);
    var str = String(node);
    if (str.match(/\n/)) return this.dump_scalar_double(str);
    if ((str.length == 0) || (str.match(/(^[ !@#%&*|\{\[]| $)/)) || (str.match(/^(~|true|false|null)$/)) || (str.match(/: /))) return this.dump_scalar_single(str);
